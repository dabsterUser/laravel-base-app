<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @if(auth()->user()->can('create users') || auth()->user()->can('manage users'))
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    @if(auth()->user()->can('edit users') || auth()->user()->can('manage users') || auth()->user()->can('delete users'))
                                    ...
                                    <tr>
                                        ...
                                        @if(auth()->user()->can('edit users') || auth()->user()->can('manage users') || auth()->user()->can('delete users'))
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                @if(auth()->user()->can('edit users') || auth()->user()->can('manage users'))
                                                ...
                                                @endif
                                                @if(auth()->user()->can('delete users') || auth()->user()->can('manage users'))
                                                    @if($user->id !== auth()->id())
