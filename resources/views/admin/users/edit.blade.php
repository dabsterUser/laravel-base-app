                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('users.update', $user) }}">
                        ...
                        <div class="mt-4 border-t border-gray-100 dark:border-gray-700 pt-4">
                            <h3 class="text-md font-medium text-gray-900 dark:text-gray-100 mb-2">{{ __('Change Password') }} <span class="text-xs text-gray-500 font-normal">({{ __('Leave blank to keep current password') }})</span></h3>
                            ...
                            <div class="mt-2 grid grid-cols-2 gap-4">
                                @foreach($roles as $role)
                                    <div class="flex items-center">
                                        <input id="role_{{ $role->id }}" type="checkbox" name="roles[]" value="{{ $role->name }}" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800" {{ is_array(old('roles', $userRoles)) && in_array($role->name, old('roles', $userRoles)) ? 'checked' : '' }}>
                                        <label for="role_{{ $role->id }}" class="ms-2 text-sm text-gray-600 dark:text-gray-400">{{ $role->name }}</label>
                                    </div>
                                @endforeach
                            </div>
