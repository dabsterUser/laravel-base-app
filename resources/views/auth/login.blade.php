<x-guest-layout>
    ...
    <div>
        ...
        <!-- Subtitle -->
        <h2 class="text-md font-bold text-slate-700 mt-6">Login to your account</h2>
    ...
    <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-5">
        ...
        <div class="flex items-center justify-between text-sm">
            <label for="remember_me" class="inline-flex items-center cursor-pointer">
                <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-[#3b59dd] shadow-sm focus:ring-[#3b59dd] dark:focus:ring-offset-gray-800 w-4 h-4" name="remember">
                <span class="ms-2 text-slate-600 font-medium">{{ __('Remember Me') }}</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-[#3b59dd] hover:underline font-semibold" href="{{ route('password.request') }}">
                    {{ __('Forgot password?') }}
                </a>
            @endif
        </div>
        ...
        <div class="text-center text-sm text-slate-500 pt-4">
            ...
            <a href="{{ route('register') }}" class="text-[#3b59dd] hover:underline font-bold">
                Create an Account
            </a>
        </div>
    </form>
